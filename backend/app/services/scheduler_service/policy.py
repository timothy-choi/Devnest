...
def can_fit_workspace_effective(
    free_cpu: float,
    free_mem: int,
    free_disk: int,
    req: WorkspaceComputeRequest,
) -> bool:
    """True if effective free CPU/RAM/disk can satisfy the workspace-shaped request."""
    if float(free_cpu) < float(req.requested_cpu):
        return False
    if int(free_mem) < int(req.requested_memory_mb):
        return False
    if int(free_disk) < int(req.requested_disk_mb):
        return False
    return True


def can_fit_workspace(node: ExecutionNode, req: WorkspaceComputeRequest) -> bool:
    """True if **allocatable** CPU/RAM/disk can satisfy the request (ignores reservations — tests / guards)."""
    ...
    if int(node.allocatable_memory_mb or 0) < int(req.requested_memory_mb):
        return False
    if int(node.allocatable_disk_mb or 0) < int(req.requested_disk_mb):
        return False
    if int(node.max_workspaces or 0) <= 0:
        return False
    return True
