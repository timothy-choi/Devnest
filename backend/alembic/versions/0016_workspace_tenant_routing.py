...
def upgrade() -> None:
    ...
    sa_sess = SaSession(bind=bind)
    try:
        rows = sa_sess.execute(sa.text("SELECT user_auth_id, username FROM user_auth")).all()
        used: set[str] = set()
        ...
        wrows = sa_sess.execute(
            sa.text(
                "SELECT workspace_id, owner_user_id, name, status FROM workspace WHERE status != 'DELETED'"
            )
        ).all()
    ...
    # Drop server_default after backfill (PostgreSQL-friendly); ignore SQLite quirks.
    try:
