...
def test_pinned_operator_workspace_name_prefix(
    ...
    monkeypatch.setenv("DEVNEST_PINNED_CREATE_EXECUTION_NODE_IDS", str(node_pk))
    get_settings.cache_clear()
...
def test_pinned_operator_create_400_when_multi_node_disabled(
    infrastructure_unit_engine: Engine,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("INTERNAL_API_KEY", "unit-test-internal-key")
    ...
    with Session(infrastructure_unit_engine) as s:
        _ensure_topology(s, 1)
        u = UserAuth(username="pin_owner_mn", email="pin_owner_mn@test", password_hash="x")
        s.add(u)
        s.commit()
        s.refresh(u)
        owner_id = int(u.user_auth_id)
        n = ExecutionNode(
            ...
            name="ec2-mn-off",
            provider_type=ExecutionNodeProviderType.EC2.value,
            provider_instance_id="i-05555555555555555",
            region="us-east-1",
            execution_mode=ExecutionNodeExecutionMode.SSM_DOCKER.value,
            status=ExecutionNodeStatus.READY.value,
            schedulable=True,
    ...
    assert "DEVNEST_ENABLE_MULTI_NODE_SCHEDULING" in (r.json().get("detail") or r.text)
    mini.dependency_overrides.clear()
    get_settings.cache_clear()


def test_pinned_operator_create_400_when_heartbeat_missing(
    infrastructure_unit_engine: Engine,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("INTERNAL_API_KEY", "unit-test-internal-key")
    ...
    with Session(infrastructure_unit_engine) as s:
        _ensure_topology(s, 1)
        u = UserAuth(username="pin_owner_hb", email="pin_owner_hb@test", password_hash="x")
        s.add(u)
        s.commit()
        s.refresh(u)
        owner_id = int(u.user_auth_id)
        n = ExecutionNode(
            ...
            name="ec2-no-hb",
            provider_type=ExecutionNodeProviderType.EC2.value,
            provider_instance_id="i-06666666666666666",
            region="us-east-1",
            execution_mode=ExecutionNodeExecutionMode.SSM_DOCKER.value,
            status=ExecutionNodeStatus.READY.value,
            schedulable=True,
    ...
    assert r.status_code == 400
    assert "heartbeat" in (r.json().get("detail") or r.text).lower()
    mini.dependency_overrides.clear()
    get_settings.cache_clear()


def test_pinned_operator_create_400_when_heartbeat_stale(
    infrastructure_unit_engine: Engine,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("INTERNAL_API_KEY", "unit-test-internal-key")
    ...
    with Session(infrastructure_unit_engine) as s:
        _ensure_topology(s, 1)
        u = UserAuth(username="pin_owner_stale", email="pin_owner_stale@test", password_hash="x")
        s.add(u)
        s.commit()
        s.refresh(u)
        owner_id = int(u.user_auth_id)
        n = ExecutionNode(
            node_key="ec2-stale-hb",
            name="ec2-stale-hb",
            provider_type=ExecutionNodeProviderType.EC2.value,
            provider_instance_id="i-07777777777777777",
            region="us-east-1",
            execution_mode=ExecutionNodeExecutionMode.SSM_DOCKER.value,
            status=ExecutionNodeStatus.READY.value,
            schedulable=True,
