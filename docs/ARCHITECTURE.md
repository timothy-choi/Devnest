...
- **Placement:** `resolve_orchestrator_placement` uses `WorkspaceRuntime.node_id` and `topology_id` for every runtime-mutating job (`START`/`STOP`/`RESTART`/`UPDATE`/`DELETE`/`RECONCILE`/`SNAPSHOT_*`/`REPO_IMPORT`) except `CREATE` (scheduler) and first `START` when no runtime row exists. `REPO_IMPORT` still resolves placement the same way but the job worker does **not** build a full orchestrator for it (git runs via the node execution bundle inside the existing container). In **staging/production**, `DEVNEST_ALLOW_RUNTIME_ENV_FALLBACK` must be **false**; legacy `DEVNEST_NODE_ID` / `DEVNEST_TOPOLOGY_ID` resolution is **development-only** when explicitly enabled. **New** scheduling (`CREATE` and first `START` reschedule paths) requires each selected `ExecutionNode.default_topology_id` to be set; env `DEVNEST_TOPOLOGY_ID` is **not** used for that decision when strict placement is on.
- **Stop/delete:** When fallback is off, the orchestrator requires a persisted Docker **engine** `container_id` (no deterministic-name guess). The same applies to **read-only health** (`check_workspace_runtime_health`) and **update** inspect/restart (no targeting by `devnest-ws-<id>` name alone in strict mode).
...
- **Durable cleanup:** Failed bring-up rollback, failed stop/detach, or IP-release debt enqueue `workspace_cleanup_task` rows. **`RECONCILE_RUNTIME`** still runs `process_durable_cleanup_tasks_for_workspace`, and **every workspace job-worker tick** also calls `drain_pending_cleanup_tasks` so cleanup is not dependent on reconcile alone. Processing is idempotent; incomplete `WorkspaceRuntime` rows get a `deferred_reason` on pending tasks instead of a fake success. Metrics: `devnest_cleanup_task_*`.
